...
import aspectminingtool.JessIntegrationModel.RedireccionFinderSeeds.RelatedCallCountedDescriptionListViewer;
import aspectminingtool.JessIntegrationModel.RedireccionFinderSeeds.SeedClassDescription;
import aspectminingtool.JessIntegrationModel.RedireccionFinderSeeds.SeedClassDescriptionListViewer;
...
import aspectminingtool.views.ViewSeedsClassesInterface;
 ...
 */
public class ViewPartClassesSeeds extends AbstractView implements ViewSeedsClassesInterface{
	public static final String ID_VIEW = "aspectminingtool.views.RedirectorFinderSeeds.ViewPartClassesSeeds"; //$NON-NLS-1$
...
//			selectAllActionCallsTable;
	...
	public final String SEED_NAME_COLUMN = "Classes Seed";
     ...
     * 
     */
	public ViewPartClassesSeeds() {
		super();
	}

	public void setName(String secondId){
		SECOND_ID = secondId;
		super.setPartName("Class Seeds - " + SECOND_ID);
	}
	
	public void addSeedClassToModel(RedirectorFinderResults redirResul, String algorithm, List<RelatedCallCountedDescription> relatedMethods,
			ProjectModel projectModel) {

		SeedClassDescription seedDescription = new SeedClassDescription();
		seedDescription.setRedirResul(redirResul);
		seedDescription.setAlgoritmo(algorithm);
		((SeedsClassGeneralModel) model).setProjectModel(projectModel);
		((SeedsClassGeneralModel) model).addMethodAsASeed(seedDescription, relatedMethods);
...
//		composite2.setLayout(layout);
	...
	private void createTableLeftColumns(Composite parent) {
		...
		.addSelectionListener(new org.eclipse.swt.events.SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				((SorterClassSeedsDescriptionView) tableViewerLeft
						.getSorter()).doSort(0);
	...
	private void createTableRightColumns(Composite parent) {
...

		{
			...
			.addSelectionListener(new org.eclipse.swt.events.SelectionAdapter() {
				public void widgetSelected(SelectionEvent event) {
					((SorterRelatedCallCountedDescription) tableViewerRight
							.getSorter()).doSort(0);
	...
	private void createTableViewerLeft() {
		...
		tableViewerLeft.setCellModifier(new CellModifierSeedClassDescription(
				this));
		...
		tableViewerLeft
				.setLabelProvider(new LabelProviderSeedsClassDescription());
	...
	private void createCallsTableViewerRight(){
		...
		// Set the cell modifier for the viewer
		tableViewerRight.setCellModifier(new CellModifierRelatedCallCounted(
				this));
		
		tableViewerRight.setContentProvider(new ContentProviderRelatedMethodsSeeds());
		tableViewerRight
				.setLabelProvider(new LabelProviderRelatedCallCounted());
	...
	protected void selectionItem(SelectionChangedEvent event) {
		...
		if (!event.getSelection().isEmpty()) {
			...
			if (event.getSelection() instanceof IStructuredSelection) {
				SeedClassDescription classDesc = (SeedClassDescription) ((IStructuredSelection) event
						.getSelection()).getFirstElement();
				tableViewerRight.setInput(classDesc);
	...
	public void createActionsTableLeft() {
		...
		deleteAction = new Action("Delete") {
			public void run() {
				...
				while (iter.hasNext()) {
					SeedClassDescription classDescription = (SeedClassDescription) iter
							.next();
					if (classDescription != null) {
						((SeedsClassGeneralModel) model)
								.removeMethodDescription(classDescription);
					}
...
public void createActionsTableRight() {
	...
	SeedClassDescriptionListViewer {
		/**
		...
		public void inputChanged(Viewer v, Object oldInput, Object newInput) {
			if (newInput != null)
				((SeedsClassGeneralModel) newInput).addChangeListenerSeedDescription(this);
			if (oldInput != null)
				((SeedsClassGeneralModel) oldInput).removeChangeListenerSeedDescription(this);
		}

		public void dispose() {
			((SeedsClassGeneralModel) model).removeChangeListenerSeedDescription(this);
		}

		// Return the methodsDescriptions as an array of Objects
		public Object[] getElements(Object parent) {
			return ((SeedsClassGeneralModel) model).getSeedDescriptions().toArray();
		}
		 ...
		 */
		public void addSeedDescription(SeedClassDescription methodDescription) {
		 ...
		 */
		public void removeSeedDescription(SeedClassDescription methodDescription) {
		 ...
		 */
		public void updateSeedDescription(SeedClassDescription methodDescription) {
	...
	RelatedCallCountedDescriptionListViewer {
		/**
		 * It register itself as a listener to the domain object changes so it can notify the tree viewer of any changes. 
		 */
		public void inputChanged(Viewer v, Object oldInput, Object newInput) {
			if (newInput != null)
				((SeedsClassGeneralModel) model).addChangeListenerRelatedMethodDescription(this);
			if (oldInput != null)
				((SeedsClassGeneralModel) model).removeChangeListenerRelatedMethodDescription(this);
		}
		...
		public Object[] getElements(Object inputElement) {
			SeedClassDescription method = (SeedClassDescription)inputElement;
			return (((SeedsClassGeneralModel)model).getRelatedMethodDescriptions(method)).toArray();
