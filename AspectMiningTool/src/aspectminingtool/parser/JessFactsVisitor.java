...
/**
 * 
 * FactsTranslatorVisitor is an extension of ASTVisitor. It defines how to visit
 * the AST nodes that are useful for the application, as well as what to do with
 * the information from each type. The class creates facts representing
 * information obtained from each AST Node. It creates the following fact:
 * 
 * Fact: method - Parameters: idMethod,methodName. Fact: returnType -
 * Parameters: Parameters: idMethod,idReturn Fact: class - Parameters:
 * idClass,className Fact: define-in - Parameters: idMethod,idClass Fact:
 * interface - Parameters: id,interfaceName Fact: abstract - Parameters:
 * idAbstract,abtractName Fact: call - Parameters: idCaller,idMethod Fact:
 * Inherits - Parameters: idClaseHija,idClasePadre //
 * idInterfaceHija,idInterfacePadre Fact: implements - Parameters:
 * idClaseHija,idInteracePadre
 * 
 * idMethod = packageName-className-methodName idClass = packageName-className
 * id = packageName-interfaceName idAbstract = packageName + abstractName
 * 
 * @author maria
 * 
 */
public class JessFactsVisitor extends FactsVisitor {
	 ...
	 * 
	 * @return facts ArrayList with the facts
	 */
	...
	/**
	 * Visits a package node and obtains the name of the package where the node
	 * belongs
	 * 
	 * @param node
	 *            the node to visit
	...
	/**
	 * Visits a TypeDeclaration AST node to obtain information about the
	 * declaration of the node (type, extends, etc)
	 * 
	 * @param node
	 *            the node to visit
	 */
	...
	/**
	 * Visits a MethodDeclaration AST node. It looks for the class and package
	 * that the method belongs, and it obtains the return type.
	 * 
	 * @param node
	 *            the node to visit
	 */
	...
	public boolean visit(MethodDeclaration node) {
		...
		// method(claveModulo,nombreModulo,tipoRetorn) claveModulo =
		// nombrePaquete-nombreClase-nombreModulo-TipoDeParametros

		this.actualMethod = actualClass + "//" + moduleName + "///" + params;
		Method m = new Method(this.actualMethod, moduleName, treturn,
				this.actualClass, params);

		// define-in(claveModulo,claveClaseContenedora)
		// Define_In define_In = new
		// Define_In(this.actualMethod,this.actualClass);
	...
	/**
	 * Visits de MethodInvocation node (the invocations of a method from another
	 * method).
	 * 
	 * @param node
	 *            the node to visit
	...
	private void getCalls(String methodName, IMethodBinding nodeBinding) {
		...
		if (nodeBinding != null) {
			...
			String className = nodeBinding.getDeclaringClass().getName(); // clase
																			// que
																			// declara
																			// el
																			// método
																			// llamado
			String packageName = nodeBinding.getDeclaringClass().getPackage()
					.getName(); // paquete que declara el método llamado

			String params = "";
			ITypeBinding[] parameters = nodeBinding.getMethodDeclaration()
					.getParameterTypes();
			for (int i = 0; i < parameters.length; i++)
				params += parameters[i].getQualifiedName() + "-";

			String calledMethod = packageName + "/" + className + "//"
					+ methodName + "///" + params;
	...
	/**
	 * Adds facts to the facts array identifying the type of the node
	 * (interface,abstract or class)
	 * 
	 * @param AST
	 *            node being visited
	 * 
	...
	private void getExtendingClasses(TypeDeclaration node) {
		...
		if ((type != null) && type.isSimpleType()) {
			...
			// Inherits(claveClaseHija,claveClaseMadre) claveClaseHija =
	...
	/**
	 * Adds the facts that defines the interfaces that the node implements and
	 * extends.
	 * 
	 * @param node
	 *            AST being visited
			...
			while (ite.hasNext()) {
				...
				if (typ.isSimpleType()) {
					...
					if (typ.resolveBinding() != null)
						interfacePackage = typ.resolveBinding().getPackage()
								.getName(); // busco el paquete al que pertenece
											// la inteface.

					String interfaceId = interfacePackage + "/"
							+ ((SimpleType) typ).getName(); // nombre completo
															// de la interface :
															// paquete + nonbre
					...
					if (node.isInterface()) // Interface extends Interface
						...
						// Clase o Abstract implements Interface
